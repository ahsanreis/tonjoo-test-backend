...
@section('content')
<form action="{{ route('edit-data-transaction') }}" method="POST" class="space-y-6">
     @csrf
     <div class="flex justify-between gap-4">
          <div class="flex flex-col w-1/2">
               <label for="description" class="w-full font-medium text-gray-700">Deskripsi:</label>
               <textarea id="description" name="description" class="w-full p-2 border border-gray-300 rounded-md" required>{{ $transaction->description ?? '' }}</textarea>
          </div>
          <div class="w-1/2">
               <div class="flex">
                    <label for="code" class="w-1/4 font-medium text-gray-700">Code:</label>
                    <input type="number" id="code" name="code" class="w-3/4 p-2 border border-gray-300 rounded-md" value="{{ $transaction->code ?? '' }}" required>
               </div>
               <div class="flex mt-4">
                    <label for="rate_euro" class="w-1/4 font-medium text-gray-700">Rate Euro:</label>
                    <input type="number" id="rate_euro" name="rate_euro" class="w-3/4 p-2 border border-gray-300 rounded-md" value="{{ $transaction->rate_euro ?? '' }}" required>
               </div>
               <div class="flex mt-4">
                    <label for="date_paid" class="w-1/4 font-medium text-gray-700">Date Period:</label>
                    <input type="date" id="date_paid" name="date_paid" class="w-3/4 p-2 border border-gray-300 rounded-md" value="{{ $transaction->date_period ?? '' }}" required>
               </div>
          </div>
     </div>
     <div class="repeater-area">
          ...
          @include('components.transactionRepeater', $detail)
          @endforeach
     </div>
